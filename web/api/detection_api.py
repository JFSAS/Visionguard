...
@detection_bp.route('/persons/<camera_id>/<int:since_frame>/<int:limit>', methods=['GET'])
def get_new_persons(camera_id, since_frame, limit):
    """获取从指定帧号后新出现的人物
    ...
    try:
        ...
        # 查找摄像头
        camera = UserCamera.query.filter_by(id=camera_id).first()
        if not camera:
            return jsonify({'success': False, 'message': f'摄像头不存在: {camera_id}'}), 404
        
        # 获取新出现的人物数据
        new_persons = DetectionService.get_new_persons(camera.id, since_frame, limit)
        ...
        logger.error(f"获取新人物数据时出错: {str(e)}")
        return jsonify({'success': False, 'message': str(e)}), 500